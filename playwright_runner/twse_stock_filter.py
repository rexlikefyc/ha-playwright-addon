...
def get_twse_stock_list():
    ...
    try:
        ...
        df = df[1:]
        df = df[df["有價證券代號及名稱"].apply(lambda x: bool(re.match(r"^\d{4}", str(x))))]
        df["stock_id"] = df["有價證券代號及名稱"].str.extract(r"^(\d{4})")
        df["name"] = df["有價證券代號及名稱"].str.extract(r"^\d{4}\s+(.+)")
        df = df[["stock_id", "name"]].dropna()
        logging.info(f"🔍 抓取到 {len(df)} 檔上市股票")
        return df
...
def fetch_twse_history(stock_id, year, month):
    ...
    try:
        ...
        if "日期" not in df.columns or "收盤價" not in df.columns or "成交股數" not in df.columns or "開盤價" not in df.columns:
        ...
        df["成交張數"] = pd.to_numeric(df["成交股數"].str.replace(",", ""), errors="coerce") / 1000
